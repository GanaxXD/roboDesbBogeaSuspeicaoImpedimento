...
where not exists(
		select 1 from client.tb_processo_tag ptag 
		inner join client.tb_tag tag on ptag.id_tag = tag.id 
									 and tag.ds_tag like 'IA_%'
		where ptag.id_processo = p.id_processo
)